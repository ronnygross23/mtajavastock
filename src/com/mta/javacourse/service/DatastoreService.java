...
public class DatastoreService {
	...
	private StockStatus fromStockEntry(Entity stockEntity) {
		...
		ret.setRecommendion(ALGO_RECOMMENDATION.valueOf((String) stockEntity.getProperty(RECOMMENDATION)));
	...
	private Entity stockToEntity(StockStatus stock) {
		...
		entity.setProperty(RECOMMENDATION, stock.getRecommendion().name());
	...
	private void updateStock(StockStatus stockStatus) {
		...
		for (Entity entity : pq.asIterable()) {
			...
			entity.setProperty(RECOMMENDATION, stockStatus.getRecommendion().name());
			datastore.put(entity);
			break;
		}
	}
